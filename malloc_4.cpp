...
void* smallocSbrk(size_t size){
    ...
    if (root == nullptr) {
        ...
        if(end->is_free){
            void* pointer=sbrk(new_size-end->size);
            if (pointer ==(void*)-1) {
                return nullptr;
            }
            end->size=new_size;
...
void* srealloc(void* oldp,size_t size) {
    ...
    if(size <= pointer->size){
        ...
        ///if pointer on wilderness chunck
        ...
        if (pointer->next->size + pointer->size + sizeof(MallocMetadata) >= size && pointer->next->is_free) {
            ...
            ///merge with both low and high address 1.d
    ...
    if(merged){
        ...
        std::memcpy((void*)(((size_t) dest) + sizeof(MallocMetadata)),oldp,tocopy);
    }
    else{  ///wilderness case included here!
        dest=smalloc(size);
        if(dest == (void*)-1){
            return nullptr;
        }
        std::memcpy(dest,oldp,tocopy);
